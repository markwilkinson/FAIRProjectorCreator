...
unless ($params[0]){
	print "<div style='background-image:url(../yukata.jpg); background-size:100%'>";
	...
	print "<br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/></div>";
	exit 1;
}
...
my $location = $1;
	...
	my $ua = LWP::UserAgent->new;	
     my $resp = $ua->get($file, Range => 'bytes=0-2999') or die;
	...
	print "<table bgcolor='#dddddd' border='1'><tr>";
	
	foreach my $b (1..scalar(@matches)+1){
		print "<td width='200px' bgcolor='yellow'>Column $b</td>";
	}
	print "</tr>\n\n";
	foreach my $a (0..3){
	print "<tr>\n\n";
			my @elems = split "\t", $lines[$a];
			foreach (@elems){
				my $brief = substr($_, 0, 45);
				if (length($brief) == 45){$brief = $brief . "..."};
				print "<td width='200px'>$brief</td>\n";
			}
	print "</tr>\n\n";
	}
	print "</table>
	...
	#print "Column # you want to FAIR Project as <em>Subject</em>: <input type='text' width='2' size='2' name='column1'/><br/><br/>\n";
	#print "Column # you want to FAIR Project as <em>Object</em>: <input type='text' width='2' size='2' name='column2'/><br/><br/>\n";
...
<div class="grid_10 omega" style="margin-left:30px">
      ...
      <h3>Subject</h3>
			   Column # you want to Project as <em>Subject</em>: <input type='text' width='2' size='2' name='column1'/><br/>
...
<br/><br/><br/>
      ...
      <h3>Object</h3>
			   ...
			   Column # you want to Project as <em>Object</em>: <input type='text' width='2' size='2' name='column2'/><br/>
...
############################################		my $file = $c->param('File');
	...
	my $ua = LWP::UserAgent->new;	
     my $resp = $ua->get($file, Range => 'bytes=0-5999') or die;
	my $content = $resp->content();
	
	my @lines = split "\n", $content;
	my $a = $lines[$headers-1];
	my @columns = split "\t", $a;
	
	my $subjectheader = $columns[$col1-1];	
	my $objectheader = $columns[$col2-1];

	$subjectheader =~ s/\s/_/g;
	$objectheader =~ s/\s/_/g;
	...
	return qq[
	{
        ...
        "store": {
                ...
                "CONSTRUCT": \["PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#> PREFIX up:<http://purl.uniprot.org/core/> ",
                        ...
                        "} ",
                        "WHERE { ",
                        "BIND(URI(CONCAT('$sstruct',?$subjectheader)) as ?url1)  ",
                        "BIND(URI(CONCAT('$ostruct',?$objectheader)) as ?url2)  ",
                        " } ",
                \],
		   "csv": "$unique",
		   "tabs": "yes",
